...
gsoep_ex2 %>%
  ...
  ungroup() %>%
  augment(m3, newdata = ., se_fit = T) %>%
  ggplot(aes(x = age*10, fill = ordered(marital), color = ordered(marital))) +
    stat_lineribbon(
      aes(ydist = dist_student_t(df = df.residual(m3), mu = .fitted, sigma = .se.fit)),
      alpha = 1/4) + 
...
```{r}
m4 <- lmer(SRhealth ~ age + marital + age:marital + (age | SID), data = gsoep_ex3)
tidy(m4)
```
